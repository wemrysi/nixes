...
-- This is setup for a 3200x1800 monitor
Config {
    ...
    commands = [
        ...
        Run Volume "hw:1" "Master" ["--","-C","#CEFFAC","-c","#FFB6B0"] 10,
    ...
    sepChar = "%",
    alignSep = "}{",
    template = "%StdinReader% }{ %cpu%   %memory%   %wlp2s0wi%: %wlp2s0%   %battery%   %hw:1:Master%   %bright%   <fc=#FFFFCC>%date%</fc>   %KORD%"
}
